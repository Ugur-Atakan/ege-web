        ...
        <>
          <dt className="text-[20px] leading-[22px] lg:text-lg">
            <Disclosure.Button className="flex w-full items-start justify-between text-left text-black">
              <span className="font-medium text-[20px] leading-[22px] lg:text-[28px] lg:leading-[32px] text-black">{t(faq.question)}</span>
          ...
          <Disclosure.Panel as="dd" className="mt-2 lg:pr-12">
            {faq.answers.map((answer, index) => (
              <p className="text-lg text-black" key={faq.id + index}>{t(answer)}</p>
            ))}
          </Disclosure.Panel>
        </>
      )}
