  ...
  return (
    <div className='bg-white'>
      <div className="mx-auto p-6 lg:px-8">
        <Link className='flex items-center gap-2' href={`/company-type`}>
          <ArrowLeftIcon className='text-[#1649FF] h-[18px] w-[18px]' />
          <span className='text-[#1649FF] text-lg font-semibold'>{'state_back_button'}</span>
        </Link>
      </div>
      <div className='mx-auto max-w-2xl px-8 md:px-0'>
        <div className='text-left md:text-center'>
          <h1 className='font-semibold  text-[26px] md:text-[40px] leading-[32px] md:leading-[44px] text-[#222222]'>{'state_title'}</h1>
        </div>
          ...
          <ul className="grid w-full gap-6 md:grid-cols-1">
            <li onClick={()=> setCompanyState('Wyoming')} className={selectedLLC === true ? 'order-1' : 'order-2'}>
              ...
              <label htmlFor="hosting-small" className={`relative inline-flex items-start justify-between w-full p-5 bg-white border border-[#C8C8C8] rounded-[20px] cursor-pointer peer-checked:border-blue-600 peer-checked:border-[4px] peer-checked:text-blue-600 ${selectedLLC === true && 'wyoming'}`}>
                <div className="block">
                  <div className="w-full text-[22px] font-semibold leading-[26px] text-[#222222]">{'state_option1_title'}</div>
                  <div className="w-[75%] pt-3 font-semibold text-lg leading-[24px] text-[#222222]">{'state_option1_text'}</div>
                </div>
            ...
            <li onClick={()=> setCompanyState('Delaware')} className={selectedLLC===true ? 'order-2' : 'order-1'}>
              ...
              <label htmlFor="hosting-big" className={`relative inline-flex items-start justify-between w-full p-5 text-gray-500 bg-white border border-[#C8C8C8] rounded-[20PX] cursor-pointer peer-checked:border-blue-600 peer-checked:border-[4px] peer-checked:text-blue-600 ${selectedLLC === false && 'delaware'}`}>
                <div className="block">
                  <div className="w-full text-[20px] md:text-[22px] font-semibold leading-[26px] text-[#222222]">{'state_option2_title'}</div>
                  <div className="w-[75%] pt-3 font-semibold text-[16px] md:text-lg leading-[24px] text-[#222222]">{'state_option2_text'}</div>
                </div>
            ...
            <li className='order-3'>
              <select
                ...
                name="companyState"
                value={companyState}
                placeholder={'state_option3_text'}
              ...
              >
                <option value="">{'state_option3_text'}</option>
            ...
            <Link href={`/company-name/`} className='order-4 w-full bg-[#1649FF] text-white text-center py-4 rounded-[20px] font-semibold text-[22px] leading-[26px] cursor-pointer'>
              <button>
              {'state_button'}
              </button>
            </Link>
